...
const String popularBooksPath = '/popularBooks';
const String topRatedBooksPath = '/top-ratedBooks';
      ...
      ShellRoute(
        ...
        routes: [
          GoRoute(
            ...
            pageBuilder:
                (context, state) => const NoTransitionPage(child: BooksView()),
            routes: [
              GoRoute(
                path: booksDetailsPath,
                name: AppRoutes.bookDetailsRoute,
                pageBuilder: (context, state) {
                  // Get both path parameter and extra
          ...
          GoRoute(
            path: novelsPath,
            name: AppRoutes.novelsRoute,
            pageBuilder:
                (context, state) => const NoTransitionPage(child: Scaffold()),
            routes: [
              GoRoute(
                path: novelsDetailsPath,
                name: AppRoutes.novelsDetailsRoute,
                pageBuilder: (context, state) {
                  // ignore: unused_local_variable
                  final bookId =
                      state.pathParameters['NovelsId'] ?? 'defaultId';
                  return CupertinoPage(child: Scaffold());
                },
              ),
              GoRoute(
                path: popularNovelsPath,
                name: AppRoutes.popularNovelsRoute,
                pageBuilder:
                    (context, state) =>
                        const NoTransitionPage(child: Scaffold()),
              ),
              GoRoute(
                path: topRatedNovelsPath,
                name: AppRoutes.topRatedNovelsRoute,
                pageBuilder:
                    (context, state) =>
                        const NoTransitionPage(child: Scaffold()),
              ),
